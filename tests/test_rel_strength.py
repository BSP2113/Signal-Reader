"""
test_rel_strength.py — test relative strength entry gate vs current baseline

Rule: at the ORB entry bar, check whether the ticker is outperforming SPY on the day.
Compare ticker % change from its day open to entry price vs SPY % change from its
day open to the same time. If ticker is not beating SPY, skip the entry.

Compares against current backfill (gap + ATR + ORB only + streak adjustment).

Run: venv/bin/python3 test_rel_strength.py
"""
...
def find_orb_entry(closes, highs, lows, volumes, times, skip_orb, spy_by_time, use_rs):
    ...
    for i in range(ORB_BARS, len(closes)):
        ...
        if closes[i] > orb_high:
            ...
            if rating != "SKIP":
                if use_rs and spy_by_time and day_open:
                    # Ticker % change from day open to entry bar
                    ticker_chg = (closes[i] - day_open) / day_open

                    # SPY % change from its day open to entry time
                    ...
                    if spy_times:
                        ...
                        if ticker_chg <= spy_chg:
                            return None   # not outperforming SPY — skip
...
def simulate_day(ticker_data, market_state, atr_modifiers, loss_streak, spy_by_time, use_rs):
    ...
    for ticker, closes, highs, lows, volumes, times, prior_close in ticker_data:
        ...
        entry = find_orb_entry(closes, highs, lows, volumes, times, skip_orb, spy_by_time, use_rs)
        if not entry:
            continue
...
def run():
    ...
    for date in dates:
        ...
        pnl = simulate_day(ticker_data, ms_map[date], atr_modifiers, loss_streak, spy_by_time, use_rs=True)
        results[date] = pnl
    ...
    # --- Results table ---
    print(f"\n{'='*60}")
    print(f"  {'Date':<12} {'Baseline':>10} {'Rel Str':>10} {'Diff':>8}")
    print(f"  {'-'*44}")
    ...
    for date in dates:
        ...
        cum_base += b
        cum_rs   += rs
        flag = " <" if abs(rs - b) > 0.50 else ""
        print(f"  {date:<12} {b:>+9.2f}  {rs:>+9.2f}  {rs-b:>+7.2f}{flag}")

    print(f"  {'-'*44}")
    print(f"  {'TOTAL':<12} {cum_base:>+9.2f}  {cum_rs:>+9.2f}  {cum_rs-cum_base:>+7.2f}")

    b_w  = sum(1 for d in dates if baseline[d] > 0)
    rs_w = sum(1 for d in dates if results[d] > 0)
    n    = len(dates)
    print(f"\n  Baseline:  {b_w}W/{n-b_w}L   ${cum_base:+.2f}")
    print(f"  Rel Str:   {rs_w}W/{n-rs_w}L   ${cum_rs:+.2f}")
    print()
