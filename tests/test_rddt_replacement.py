"""
test_rddt_replacement.py — Find a replacement for RDDT (21% win rate, +$16.13 but
dependent on one $33.97 trade; without it = -$17.84).

Tests HOOD, HIMS, AFRM, SNAP through EX1 over the full 53-day window
(38-day backfill + 15 live days).

Does NOT touch exercises.json or backfill.json. Uses a temp file, then deletes it.

Run: venv/bin/python3 test_rddt_replacement.py
"""
...
CANDIDATES = ["HOOD", "HIMS", "AFRM", "SNAP"]

# 38-day backfill window
DATES_BACKFILL = [
...
# 15 live days (Apr 27+ are not in backfill)
DATES_LIVE = [
    "2026-04-27","2026-04-28","2026-04-29","2026-04-30","2026-05-01",
]

ALL_DATES = DATES_BACKFILL + DATES_LIVE  # 43 unique dates (backfill already covers Apr 13-24)

TEMP_FILE = "_test_rddt_replacement.json"
...
def run():
    ...
    print(f"\nTesting RDDT replacements: {', '.join(CANDIDATES)}")
    print(f"Window: {ALL_DATES[0]} → {ALL_DATES[-1]}  ({len(ALL_DATES)} days)\n")
    print("(RDDT baseline: 14 trades, 21% WR, +$16.13 — but $33.97 of that is one trade; ex-that = -$17.84)")
