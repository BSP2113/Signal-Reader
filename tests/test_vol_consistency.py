"""
test_vol_consistency.py — test volume consistency check vs ORB-only baseline

Rule: at the ORB breakout bar, require that at least 2 of the 3 prior bars also
had above-average volume. A single volume spike can be a one-off print; a
sustained build is a more reliable signal.

Compares against current backfill (gap filter + ATR sizing + ORB only + streak adj).

Run: venv/bin/python3 test_vol_consistency.py
"""
...
VOL_PRIOR_BARS = 3     # how many bars before entry to check
VOL_PRIOR_MIN  = 2     # how many of those must be above average
...
def find_orb_entry(closes, highs, lows, volumes, times, skip_orb, use_vol_check):
    ...
    for i in range(ORB_BARS, len(closes)):
        ...
        if closes[i] > orb_high:
            ...
            if rating != "SKIP":
                if use_vol_check and i >= VOL_PRIOR_BARS:
                    prior_above = sum(
                        1 for k in range(1, VOL_PRIOR_BARS + 1)
                        if volumes[i - k] >= avg_vol
                    )
                    if prior_above < VOL_PRIOR_MIN:
                        return None   # volume build not sustained — skip
...
def simulate_day(ticker_data, market_state, atr_modifiers, loss_streak, use_vol_check):
    ...
    for ticker, closes, highs, lows, volumes, times, prior_close in ticker_data:
        ...
        entry = find_orb_entry(closes, highs, lows, volumes, times, skip_orb, use_vol_check)
        if not entry:
            continue
...
def run():
    print("Loading current backfill (baseline with streak adjustment)...")
    ...
    for date in dates:
        ...
        pnl = simulate_day(ticker_data, ms_map[date], atr_modifiers, loss_streak, use_vol_check=True)
        results[date] = pnl
    ...
    # --- Results table ---
    print(f"\n{'='*60}")
    print(f"  {'Date':<12} {'Baseline':>10} {'Vol Check':>10} {'Diff':>8}")
    print(f"  {'-'*44}")

    cum_base = cum_vol = 0.0
    for date in dates:
        b = baseline[date]
        v = results[date]
        cum_base += b
        ...
        print(f"  {date:<12} {b:>+9.2f}  {v:>+9.2f}  {v-b:>+7.2f}{flag}")

    print(f"  {'-'*44}")
    print(f"  {'TOTAL':<12} {cum_base:>+9.2f}  {cum_vol:>+9.2f}  {cum_vol-cum_base:>+7.2f}")

    b_w = sum(1 for d in dates if baseline[d] > 0)
    v_w = sum(1 for d in dates if results[d] > 0)
    n   = len(dates)
    print(f"\n  Baseline:  {b_w}W/{n-b_w}L   ${cum_base:+.2f}")
    print(f"  Vol Check: {v_w}W/{n-v_w}L   ${cum_vol:+.2f}")
    print()
