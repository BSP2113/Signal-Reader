"""
test_trail_lock.py — test raising TRAIL_LOCK from 1% to 2% vs current baseline

Problem: 5 trailing stop trades in backfill, 4 of them losses. All fired after
the price only moved +0.9–1.6% — barely above the 1% lock level. The trail
then let the price drop 2.5% from that tiny peak, exiting near or below entry.

Fix: raise TRAIL_LOCK to 2%. The trailing stop only activates after a real gain.
Trades that peaked under 2% fall through to the regular stop loss (-1.5%) or
time close instead.

Compares against current backfill (all gates active).

Run: venv/bin/python3 test_trail_lock.py
"""
...
TRAIL_LOCK_BASE = 0.01   # current: 1%
TRAIL_LOCK_NEW  = 0.02   # test:    2%
...
def find_exit(closes, times, entry_price, entry_bar, trail_lock):
    peak = entry_price
...
def simulate_day(ticker_data, market_state, atr_modifiers, loss_streak,
                 spy_by_time, in_drawdown, trail_lock):
    ...
    for ticker, closes, highs, lows, volumes, times, prior_close in ticker_data:
        ...
        cash -= alloc
        exit_p, exit_r = find_exit(closes, times, entry["price"], entry["bar"], trail_lock)
...
def run():
    ...
    print(f"  Testing TRAIL_LOCK 1% (current) vs 2% (new)\n")
    ...
    for date in dates:
        ...
        in_drawdown = portfolio_drawdown(completed_dates, results_base)

        pnl_base = simulate_day(ticker_data, ms_map[date], atr_modifiers, loss_streak,
                                spy_by_time, in_drawdown, TRAIL_LOCK_BASE)
        pnl_new  = simulate_day(ticker_data, ms_map[date], atr_modifiers, loss_streak,
                                spy_by_time, in_drawdown, TRAIL_LOCK_NEW)
        ...
        diff = pnl_new - pnl_base
        flag = f"  ({diff:+.2f})" if abs(diff) > 0.01 else ""
        print(f" base {pnl_base:+.2f}  new {pnl_new:+.2f}{flag}")

    # --- Results table ---
    print(f"\n{'='*64}")
    print(f"  {'Date':<12} {'Baseline':>10} {'Lock 2%':>10} {'Diff':>8}")
    print(f"  {'-'*46}")

    cum_base = cum_new = 0.0
    for date in dates:
        b = results_base[date]
        n = results_new[date]
        cum_base += b
        cum_new  += n
        flag = " <" if abs(n - b) > 0.50 else ""
        print(f"  {date:<12} {b:>+9.2f}  {n:>+9.2f}  {n-b:>+7.2f}{flag}")

    print(f"  {'-'*46}")
    print(f"  {'TOTAL':<12} {cum_base:>+9.2f}  {cum_new:>+9.2f}  {cum_new-cum_base:>+7.2f}")

    b_w = sum(1 for d in dates if results_base[d] > 0)
    n_w = sum(1 for d in dates if results_new[d] > 0)
    nd  = len(dates)
    print(f"\n  Baseline (1%): {b_w}W/{nd-b_w}L   ${cum_base:+.2f}")
    print(f"  Lock 2%:       {n_w}W/{nd-n_w}L   ${cum_new:+.2f}")
    print()
