"""
test_rklb_replacement.py — Find a replacement for RKLB (removed: 29% win rate, -$23.61 over 17 trades).

Tests HOOD, HIMS, and SOUN through EX1 over the 30-day window.
Does NOT touch exercises.json. Uses a temp file, then deletes it.

Run: venv/bin/python3 test_rklb_replacement.py
"""
...
TEMP_FILE = "_test_rklb_replacement4.json"
...
def run():
    ...
    print(f"\nTesting RKLB replacements: {', '.join(CANDIDATES)}")
    print(f"Dates: {DATES_30[0]} → {DATES_30[-1]}  ({len(DATES_30)} days)\n")
    print("(RKLB baseline for comparison: 17 trades, 29% win rate, -$23.61 combined)")
    ...
    print(f"\n{'='*70}")
    print("  PER-TICKER BREAKDOWN  (30-day window)")
    print(f"{'='*70}")
    ...
    for ticker in CANDIDATES:
        ...
        if not trades:
            print(f"  {ticker:<8} {'(no signals)':>50}")
        ...
        ticker_results[ticker] = {"trades": len(trades), "wins": len(wins), "wr": wr, "pnl": total, "avg_w": avg_w, "avg_l": avg_l}
    ...
    for ticker, r in ticker_results.items():
        if r is None:
            print(f"  {ticker}: no signals fired — not a good ORB candidate")
            continue
        flags = []
        if r["wr"] >= 55:
            flags.append("strong win rate")
        elif r["wr"] >= threshold_wr:
            flags.append("acceptable win rate")
        else:
            flags.append("low win rate")
        if r["pnl"] > 50:
            flags.append("solid P&L")
        elif r["pnl"] > threshold_pnl:
            flags.append("marginal P&L")
        else:
            flags.append("negative P&L")
        verdict = "CANDIDATE" if r["wr"] >= threshold_wr and r["pnl"] > threshold_pnl else "REJECT"
